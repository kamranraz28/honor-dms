  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active"><a href="{{ route('sales.distributorSales') }}">Distributor Sale</a></li>
      </ol>
    </section>
    ...
    <section class="content-header">
      ...
      <div class="box box-warning">
            ...
            <!-- /.box-header -->
            <!-- form start -->
             <form class="form-horizontal" method="POST" action="{{ route('sales.distributorSales.store') }}" autocomplete="on" enctype="multipart/form-data">
<div class="box-body">
    @if(count($errors))
...
@foreach ($distsales as $element)
  ...
  <button class="btn btn-xs btn-primary" data-toggle="modal" data-target="#{{'distsalesUpdateModal'. $element['id']}}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>

  <button class="btn btn-xs btn-danger" data-toggle="modal" data-target="#{{'distsalesDeleteModal'. $element['id']}}"><i class="fa fa-trash-o" aria-hidden="true"></i></button>
...
@forelse ($distsales as $key => $element)
  <!-- Modal -->
  <div class="modal fade" id="{{'distsalesUpdateModal'. $element['id']}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<form action="{{route('sales.distributorSales.update')}}" method="post">
...
<div class="form-group {{ $errors->has('product_qty') ? 'has-error' : '' }}">
  <label for="product_qty">Specility:</label>
  <input type="number" id="product_qty" name="product_qty" class="form-control" placeholder="Enter Product Name" value="{{ $element['product_qty'] }}">
...
@forelse ($distsales as $key => $element)
  <!-- Modal -->
  <div class="modal fade" id="{{'distsalesDeleteModal'. $element['id']}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
...
<!-- body part -->
  ...
  <form action="{{route('sales.distributorSales.delete',$element['id'])}}" method="post">
