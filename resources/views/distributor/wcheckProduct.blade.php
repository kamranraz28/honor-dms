...
<!-- ==============one section area ================= -->
  ...
  <section class="col-lg-12 connectedSortable">
          ...
          <div class="box box-warning">
            ...
            <!-- form start -->
             <form class="form-horizontal1" method="POST" action="{{ route('distributor.wcheckProduct.store') }} " autocomplete="off" enctype="multipart/form-data">
...
{{-- for for displaying success and errror message --}}
        ...
        <div class="col-md-8">
          <div class="form-group {{ $errors->has('imei') ? 'has-error' : '' }}">
            <label for="imei">IMEI 1 Or IMEI 2:</label>
        ...
        <div class="col-md-4">
          <div class="form-group">
            <label for="model" style="visibility: hidden;">IMEI 1  Or IMEI 2:</label>
...
<form action="{{route('distributor.wcheckProduct.repalce')}}" method="post" enctype="multipart/form-data">
...
<div class="form-group">
 ...
 <select name="problem" id="problem" class="form-control" required="required>
  ...
  <option value="Charger Issue">Charger Issue</option>
    ...
    <option value="Network problem/Call Drop/Call issue ">Network problem/Call Drop/Call issue </option>
      <option value="Suddenly display white/Display Line/Display Blinking issue ">Suddenly display white/Display Line/Display Blinking issue </option>
  <option value="Suddenly off/Auto ON OFF">Suddenly off/Auto ON OFF</option>
  <option value="No charge/Charge Not Stay/Charge Backup issue">No charge/Charge Not Stay/Charge Backup issue</option>
  <option value="Sim slot issue">Sim slot issue</option>
  <option value="Device Hang/ Heating issue">Device Hang/ Heating issue</option>
...
<div class="form-group">
  <label for="sno">Problem (others - if not available in previous list):</label>
  <input type="text" id="problem_other" name="problem_other" class="form-control" placeholder="Enter problem">
</div>

<div class="form-group {{ $errors->has('sno') ? 'has-error' : '' }}">
  <label for="sno">Replaced IMEI 1</label>
...
<div class="form-group {{ $errors->has('imei') ? 'has-error' : '' }}">
  <label for="imei">Replaced IMEI 2:</label>
...
<div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                  <label for="inputPassword3" class="col-sm-2 control-label">Upload Memo</label>
                  <div class="col-sm-10">
                    <input class="form-control" name="image" type="file" required="required>
                    <span class="text-danger">{{ $errors->first('image') }}</span>
                  </div>
                  <label for="inputPassword3" class="col-sm-8 control-label" style="color: red;">Maximum File Size 400 KB</label>
                </div>

  <div class="form-group">
    <button class="form-control btn btn-success">Replace</button>
  </div>
</form>
...
<table class="table"> 
  ...
  <tbody>
    @foreach ($wcheckProduct['replace'] as $key => $replace)
      <tr>
        <td>{{$key + 1}}</td>
        <td>{{$replace['contact_name']}} </td>
        <td>{{$replace['contact_no']}} </td>
        <td>{{$replace['service_type']}} </td>
        <td>{{$replace['problem']}} </td>
        <td>
          @if ($replace['imei'])
            {{$replace['imei']}}
          @else
        ...
        <td>
        <a target="_blank" href="{{ asset( 'storage/app/' . $replace['memo']) }}">
    View Image
  </a>
</td>
<!-- <td>
