...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <div class="row new-box">
                    ...
                    <table class="table-bordered table">
                        <thead>
                            <tr>
                                ...
                                <th colspan="3">Date: {{ $orader->created_at->format('Y-m-d') }} <br> Time: {{
                                    $orader->created_at->format('H:i:s') }} </th>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <th colspan="3">Dist Address: {{ $orader->users->address ?: '-' }}</th>
            ...
            <div class="col-sm-6">
                <h1 class="orader">Approved order</h1>
                <table class="table-bordered table">
                    <thead>
                        <tr>
                            <th colspan="5">Order Number: {{ $orader->id }}
                                <input type="hidden" name="orader_number" value="{{ $orader->id }}">
                            </th>
                            <th colspan="2">Date: {{ $orader->created_at->format('Y-m-d') }} <br> Time: {{
                                $orader->created_at->format('H:i:s') }} </th>
                        </tr>
                        <tr>
                            <th colspan="5">Dist Name: {{ @$orader->users->firstname }}</th>
                            <th colspan="2">Dist code: {{ @$orader->users->officeid }}</th>
                        </tr>
                        <tr>
                            <th colspan="5">Dist Address: {{ $orader->users->address ?: '-' }}</th>
                            <th colspan="2">Status:
                                @switch($orader->status)
                                @case(0)
                                <strong>Drft</strong>
                    ...
                    <tbody>
                        ...
                        @for ($count = 0; $count < count($value->OrderspostingDetails); $count++)
                            <tr>
                                <td>{{ $i++ }}</td>
                                <td>{{ $value->OrderspostingDetails[$count]->product->model }}</td>
                                <td>@moneybdt($value->OrderspostingDetails[$count]->price)</td>
                                <td>{{ $value->OrderspostingDetails[$count]->quantity }}</td>
                                <td>@moneybdt($value->OrderspostingDetails[$count]->price *
                                    $value->OrderspostingDetails[$count]->quantity)</td>
                                <td>@moneybdt($value->OrderspostingDetails[$count]->price_acc)</td>
                                <td>@moneybdt(($value->OrderspostingDetails[$count]->price *
                                    $value->OrderspostingDetails[$count]->quantity) -
                                    $value->OrderspostingDetails[$count]->price_acc)</td>
                                @php
                                $subtotal = $value->OrderspostingDetails[$count]->price *
                                $value->OrderspostingDetails[$count]->quantity;
                ...
                <h4>Remarks:</h4> <b>{{ $postings->first()->remarks ?? '-' }}</b>
                <h4>Delivery Info:</h4> <b>{{ $postings->first()->delivery_info ?? 'Pending' }}</b>
            </div>
            @endif
