  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active"><a href="{{ route('sales.crudDistributorTarget.store') }}">Edit/Delete Distributor Target</a></li>
      </ol>
    </section>
    ...
    <section class="content-header">
      ...
      <div class="box box-warning">
            ...
            <!-- /.box-header -->
            <!-- form start -->
  <form class="form-horizontal" method="POST" action="{{ route('sales.crudDistributorTarget.store') }}" autocomplete="on" enctype="multipart/form-data">
<div class="box-body">
    @if(count($errors))
...
@foreach ($targets as $element)
  <tr>
    ...
    <td>{{$element['distributor_id'] }}</td>
    <td>{{$element['date'] }}</td>
    <td>{{$element['product_sku'] }}</td>
    <td>{{$element['product_qty'] }}</td>
     <td>{{$element['product_qty']*$element['product_price']}}TK</td>
 
                  
                  <td>

  <button class="btn btn-xs btn-primary" data-toggle="modal" data-target="#{{'targetListUpdateModal'. $element['id']}}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>

  <button class="btn btn-xs btn-danger" data-toggle="modal" data-target="#{{'targetListDeleteModal'. $element['id']}}"><i class="fa fa-trash-o" aria-hidden="true"></i></button>
...
@forelse ($targets as $key => $element)
  <!-- Modal -->
  <div class="modal fade" id="{{'targetListUpdateModal'. $element['id']}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<form action="{{route('sales.crudDistributorTarget.update')}}" method="post">
...
@forelse ($targets as $key => $element)
  <!-- Modal -->
  <div class="modal fade" id="{{'targetListDeleteModal'. $element['id']}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
...
<!-- body part -->
  ...
  <form action="{{route('sales.crudDistributorTarget.delete',$element['id'])}}" method="post">
