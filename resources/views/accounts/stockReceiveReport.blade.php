        ...
        <section class="col-lg-12 connectedSortable">
            ...
            <div class="box box-warning">
                <div class="box-header">
                    <h3 class="box-title text-danger">Stock Receive Report</h3>
                </div>

                <div class="box-body">
                        <form action="{{route('accounts.stockReceiveReportStore')}}" method="post">
                            @csrf
                ...
                <div class="box-header">
                    <table id="example" class="ui celled table" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            @foreach($stockReceiveReport['productModel'] as $key => $productModel)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $stockReceiveReport['productDetailsWithCounts'][$productModel]['name'] }}</td>
                                <td>{{ $stockReceiveReport['productDetailsWithCounts'][$productModel]['color'] }}</td>
                                <td>{{ $stockReceiveReport['productDetailsWithCounts'][$productModel]['count'] ?? 0 }}
                                </td>
                            </tr>
                            @endforeach
...
@php
  Session::forget(['stockReceiveReport','fdate', 'todate']);
@endphp
