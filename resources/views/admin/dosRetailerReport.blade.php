  ...
  <section class="col-lg-12 connectedSortable">
          ...
          <div class="box box-warning">
            <div class="box-header">
              <h3 class="box-title text-danger">DOS Report</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
  
            <!-- form start -->
             <form class="form-horizontal" method="POST" action="{{ route('admin.dosRetailerReport.store') }} " autocomplete="off" enctype="multipart/form-data">
...
{{-- for for displaying success and errror message --}}
               ...
               <br><br>
                ...
                <div class="col-md-8">
                  <label class="control-label" for="distributor">Retailer :</label>
...
<table id="example" class="ui celled table" width="100%">
    ...
    <tbody>
    @foreach ($dosReports as $key => $element)
<tr>
          <td> {{$key + 1}} </td>
          <td>{{ $element['distributor'] ?: 'All Distributor' }}</td>
          <td> {{$element['product']}} - {{$element['model']}}</td>
          <td> {{$element['stockin']}} </td>
          <td> {{$element['stockout']}} </td>
          <td> {{$element['stock']}} </td>
          <td>{{$element['lastmonth_sale']}}</td>
          <td>{{ round($element['lastmonth_sale'] ? ($element['stock'] / ($element['lastmonth_sale'] / 30)) : 0) }}</td>
