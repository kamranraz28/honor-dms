...
@section('title')
  {{"Sales Automation Process :: Category"}}
@endsection
...
{{-- for for displaying success and errror message --}}
  <form class="form-horizontal" method="POST" action="{{ route('categories.store') }}" autocomplete="on" enctype="multipart/form-data">
<div class="box-body">
    @if(count($errors))
...
@foreach ($cats as $key=>$element)
                ...
                <tr>
                  ...
                  <td>
                <!--
                   <a href="#" class=" btn btn-warning btn-md" data-toggle="modal" data-target="#{{'catUpdateModal'. $element['id']}}">Edit</a>
                   <a href="#" class=" btn btn-danger btn-md" data-target="#{{'catDeleteModal'. $element['id']}}">Delete</a>
                 -->

  <button class="btn btn-xs btn-primary" data-toggle="modal" data-target="#{{'catUpdateModal'. $element['id']}}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>
...
@forelse ($cats as $key => $element)
  <!-- Modal -->
  <div class="modal fade" id="{{'catUpdateModal'. $element['id']}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<form action="{{route('categories.update',$element['id'])}}" method="post">
